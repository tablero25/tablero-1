  ...
  return (
    <div className="tablero-bg">
      <div style={{textAlign:'center', padding:'30px 0', color:'#fff'}}>
        <h1 style={{fontSize:'2.5rem', margin:0, fontWeight:'bold'}}>TABLERO DE CONTROL</h1>
        <h2 style={{fontSize:'1.8rem', margin:'10px 0', opacity:0.9}}>INDICADOR DE GESTIÓN</h2>
      </div>
...
function IndicadoresCamas({ user }) {
  ...
  // Si el usuario es admin o director, mostrar todos los establecimientos
  // Si es gerente, mostrar solo su establecimiento
  // Si es jefe de zona, mostrar sus establecimientos asignados
  const isAdmin = user && user.rol === 'admin';
  const isDirector = user && user.rol === 'director';
  const isJefeZona = user && user.rol === 'jefe de zona';
  ...
  if (isAdmin || isDirector) {
    ...
    // Para jefe de zona, mostrar solo sus establecimientos asignados
    zonasFiltradas = ZONAS.map(zona => ({
      ...zona,
      establecimientos: zona.establecimientos.filter(est => 
        user.establecimientosAsignados && user.establecimientosAsignados.some(estAsignado => 
          est.includes(estAsignado)
        )
      )
    })).filter(zona => zona.establecimientos.length > 0);
  } else {
    // Para gerente, mostrar solo su establecimiento
    zonasFiltradas = ZONAS.map(zona => ({
      ...zona,
      establecimientos: zona.establecimientos.filter(est => 
      ...
      <div style={{textAlign:'center', padding:'30px 0', color:'#fff'}}>
        ...
        {!isAdmin && !isDirector && (
          <p style={{fontSize:'1rem', margin:'10px 0', opacity:0.8}}>
            {isJefeZona ? `Establecimientos asignados: ${user.establecimientosAsignados.join(', ')}` : `Establecimiento: ${user.establecimiento}`}
          </p>
        )}
      </div>
...
function IndicadoresCamasEstablecimiento({ user }) {
  ...
  useEffect(() => {
          fetch(`${API_ENDPOINTS.LISTAR_ARCHIVOS}/${encodeURIComponent(nombreEstablecimiento)}`)
      .then(res => res.json())
  ...
  useEffect(() => {
            fetch(`${API_ENDPOINTS.LISTAR_ARCHIVOS}/${encodeURIComponent(nombreEstablecimiento)}/${anio}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    if (archivoSeleccionado) {
      // Cargar datos del archivo guardado
              fetch(`${API_ENDPOINTS.LEER_ARCHIVO}/${encodeURIComponent(nombreEstablecimiento)}/${anio}/${archivoSeleccionado}`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
  ...
  const actualizarArchivos = (establecimiento, anio) => {
          fetch(`${API_ENDPOINTS.LISTAR_ARCHIVOS}/${encodeURIComponent(establecimiento)}/${anio}`)
      .then(res => res.json())
  ...
  const handleGuardar = async () => {
    ...
    try {
      const url = `${API_ENDPOINTS.GUARDAR_ARCHIVO}/${encodeURIComponent(nombreEstablecimiento)}/${anio}`;
...
function RankingDiagnosticoEstablecimiento({ user }) {
  const { nombre } = useParams();
  const navigate = useNavigate();
  const nombreEstablecimiento = decodeURIComponent(nombre);
...
function RankingDiagnosticoCategoria({ user }) {
...
function App() {
  ...
  const handleLogin = (userData) => {
    setUser(userData);
    // Asegurar que el usuario sea redirigido a /tablero después del login
    // La redirección se maneja automáticamente por el Route con Navigate
  };
  ...
  if (loading) {
    return (
      <div className="tablero-bg" style={{
        display: 'flex',
        alignItems: 'center',
  ...
  // Si no hay usuario logueado, mostrar página de inicio o autenticación
  if (!user) {
    return (
      <Routes>
        <Route path="/" element={<Welcome />} />
        <Route path="/login" element={<Auth onLogin={handleLogin} />} />
        <Route path="/registro" element={<Auth onLogin={handleLogin} />} />
        <Route path="*" element={<Welcome />} />
      </Routes>
    );
  }
  ...
  return (
    <div className="tablero-bg">
      ...
      <Routes>
        ...
        <Route path="/tablero" element={<Home user={user} />} />
        ...
        <Route path="/ranking-diagnostico/:nombre" element={<RankingDiagnosticoEstablecimiento user={user} />} />
        <Route path="/ranking-diagnostico/:nombre/:categoria" element={<RankingDiagnosticoCategoria user={user} />} />
      </Routes>
