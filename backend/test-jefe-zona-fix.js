...
console.log('🧪 PROBANDO CAMBIO A ROL JEFE DE ZONA SIN ESTABLECIMIENTOS\n');

async function testJefeZonaRole() {
  try {
    ...
    db.get('SELECT * FROM usuarios WHERE dni = ?', ['admin'], async (err, adminUser) => {
      ...
      db.get('SELECT * FROM usuarios WHERE dni != ? LIMIT 1', ['admin'], async (err, testUser) => {
        ...
        // Probar cambio a jefe de zona sin establecimientos
        const testData = {
          userId: testUser.id,
          rolId: 7 // Jefe de zona
          // Sin establecimientosSeleccionados
        };

        console.log('🧪 Probando cambio a jefe de zona sin establecimientos');
        ...
        if (response.status === 200) {
          console.log('✅ ¡Cambio a jefe de zona exitoso!');
          ...
          console.log('❌ Error al cambiar a jefe de zona');
          db.close();
        }
      });
    });
